...
const { getBlizzardToken } = require('../utils/wowApi');
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const [profileRes, pvpRes, rioRes] = await Promise.all([
        ...
        fetch(`https://raider.io/api/v1/characters/profile?region=${region}&realm=${encodeURIComponent(realm)}&name=${encodeURIComponent(name)}&fields=gear`),
      ]);
      ...
      const embed = new EmbedBuilder()
        .setTitle(`⚔️ Stats PvP — ${profile?.name ?? rioData?.name ?? name} (${realm})`)
        .setColor(color)
